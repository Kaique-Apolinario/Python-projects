...
def draw(bp):
    if homage in "[E]" or homage == "[e]" and bp == 'IS':
        print("-"*20)
...
while True:
    while True:
        # This is just a silly play with BLACKPINK's (Girl Group that I like) catchphrase "BLACKPINK in your area!".
        # Please, don't take it too seriously.
        # Here's the code to enable and disable the homage:
        homage = input("First, enable or disable the BLACKPINK's homage? [D] for 'Disable' and [E] for 'Enable': ")
    ...
    while True:
        try:
            len_wid = [float(input(f"Type the length of the room {room}: ")), float(input("Now, the width: "))]
        except:
    ...
    for wait in range(0, 6):
        sleep(1)
        print(".", end='')
    print(f"\nThe room's area is {area(len_wid):.2f}m²", end='' f" and BLACKPINK {bp} in your area, look:\n"
